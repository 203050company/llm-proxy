...
describe("translateGeminiToCodexRequest", () => {
  ...
  it("preserves inlineData that is already a data URL", () => {
    const result = translateGeminiToCodexRequest(
      makeRequest({
        contents: [
          {
            ...
            parts: [
              { text: "Describe this" },
              { inlineData: { mimeType: "image/png", data: "data:image/png;base64,abc123" } },
            ],
          },
        ],
      }),
    ...
    const userItem = result.input[0];
    const content = (userItem as { content: unknown }).content;
    const parts = content as Array<{ type: string; image_url?: string }>;
    expect(parts[1]).toEqual({
      type: "input_image",
      image_url: "data:image/png;base64,abc123",
    });
