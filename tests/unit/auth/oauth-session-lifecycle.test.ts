...
describe("OAuth session lifecycle", () => {
  ...
  it("passes callback server id token to account handler", async () => {
    vi.mocked(curlFetchPost).mockResolvedValue({
      ok: true,
      status: 200,
      body: JSON.stringify({
    ...
    const { createOAuthSession, startCallbackServer } = await import("@src/auth/oauth-pkce.js");
    const { state } = createOAuthSession("localhost:8080", "login");
    const onAccount = vi.fn();
    const server = startCallbackServer(0, onAccount);
    await once(server, "listening");
    const port = (server.address() as AddressInfo).port;

    const response = await fetch(`http://localhost:${port}/auth/callback?code=abc&state=${state}`);

    expect(response.status).toBe(200);
    expect(onAccount).toHaveBeenCalledWith("access-token", "refresh-token", "real-id-token");
    server.close();
  });
});
