...
describe("AccountPool", () => {
  ...
  describe("basic acquire/release", () => {
    ...
    it("stores and updates the real id token", () => {
      const id = pool.addAccount("token-aaa", "rt-old", "id-token-old");

      expect(pool.getEntry(id)?.idToken).toBe("id-token-old");

      const sameId = pool.addAccount("token-aaa-new", "rt-new", "id-token-new");

      expect(sameId).toBe(id);
      expect(pool.getEntry(id)?.idToken).toBe("id-token-new");
    });

    it("clears stale refresh failure when an existing account is re-added", () => {
      const id = pool.addAccount("token-aaa", "rt-old");
      pool.recordRefreshFailure(id, "recovery", null, "refresh token already used");

      const sameId = pool.addAccount("token-aaa-new", "rt-new");

      const account = pool.getAccounts().find((a) => a.id === id);

      expect(sameId).toBe(id);
      expect(account?.lastRefreshFailureAt).toBeNull();
      expect(account?.lastRefreshFailureCode).toBeNull();
      expect(pool.getEntry(id)?.refreshSummary.lastRefreshFailureMessage).toBeNull();
    });
  ...
  describe("account metadata", () => {
    it("updates an account plan type", () => {
      const id = pool.addAccount("token-free-plan-refresh");

      expect(pool.getAccounts().find((account) => account.id === id)?.planType).toBe("free");

      pool.updatePlanType(id, "plus");

      expect(pool.getAccounts().find((account) => account.id === id)?.planType).toBe("plus");
