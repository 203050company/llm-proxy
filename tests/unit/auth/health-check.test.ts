...
let refreshResult: { access_token: string; refresh_token: string | null; id_token?: string } | Error = {
  access_token: "",
...
describe("probeAccount", () => {
  ...
  it("stores id token returned by manual refresh probe", async () => {
    const validToken = makeValidJwt();
    refreshResult = {
    ...
    await probeAccount(pool as never, scheduler as never, "acc-1");

    expect(pool.updateToken).toHaveBeenCalledWith("acc-1", validToken, "new_rt", "real-id-token");
  });
  ...
  it("returns dead and marks expired when OpenAI invalidates the refresh-token chain", async () => {
    refreshResult = new Error(
    ...
    expect(result.result).toBe("dead");
    expect(result.error).toContain("refresh token has been invalidated");
    expect(pool.markStatus).toHaveBeenCalledWith("acc-1", "expired");
  });
