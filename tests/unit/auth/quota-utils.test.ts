...
describe("toQuota", () => {
  ...
  it("uses nested rate_limit plan_type when top-level plan_type is missing", () => {
    const usage = makeUsageResponse();
    delete (usage as Partial<CodexUsageResponse>).plan_type;
    (usage.rate_limit as { plan_type?: string }).plan_type = "plus";
  ...
  it("converts flat rate_limit usage responses", () => {
    const quota = toQuota({
      rate_limit: {
        plan_type: "plus",
        allowed: true,
    ...
    expect(quota.plan_type).toBe("plus");
    expect(quota.rate_limit.used_percent).toBe(27);
    expect(quota.rate_limit.reset_at).toBe(1700003600);
    expect(quota.rate_limit.limit_window_seconds).toBe(18000);
    expect(quota.rate_limit.allowed).toBe(true);
    expect(quota.rate_limit.limit_reached).toBe(false);
  });
