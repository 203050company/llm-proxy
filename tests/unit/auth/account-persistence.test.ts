...
describe("account-persistence", () => {
  ...
  describe("load", () => {
    ...
    it("preserves real id tokens from accounts.json", () => {
      const entry = makeEntry("a");
      entry.idToken = "real-id-token";
      const data: AccountsFile = { accounts: [entry] };
      mockFs.existsSync.mockImplementation(((path: string) =>
        path.includes("accounts.json")) as () => boolean,
      );
      ...
      expect(result.entries[0].idToken).toBe("real-id-token");
    });

    it("backfills missing idToken to null and auto-persists", () => {
      const entry = makeEntry("a");
      delete (entry as Partial<AccountEntry>).idToken;
      const data: AccountsFile = { accounts: [entry] };
      mockFs.existsSync.mockImplementation(((path: string) =>
        path.includes("accounts.json")) as () => boolean,
      );
      ...
      expect(result.entries[0].idToken).toBeNull();
