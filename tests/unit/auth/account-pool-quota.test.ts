...
describe("AccountPool quota methods", () => {
  ...
  describe("updateCachedQuota", () => {
    ...
    it("updates plan type from authoritative quota data", () => {
      const id = pool.addAccount(createValidJwt({ accountId: "a1-plan", planType: "free" }));

      pool.updateCachedQuota(id, makeQuota({ plan_type: "plus" }));

      expect(pool.getEntry(id)?.planType).toBe("plus");
    });

    it("does not let a refreshed JWT free claim downgrade a quota-confirmed plus account", () => {
      const id = pool.addAccount(createValidJwt({ accountId: "a1-refresh", planType: "free" }));
      pool.updateCachedQuota(id, makeQuota({ plan_type: "plus" }));

      pool.updateToken(id, createValidJwt({ accountId: "a1-refresh", planType: "free" }));

      expect(pool.getEntry(id)?.planType).toBe("plus");
    });

    it("does not let a refreshed JWT free claim downgrade an existing non-free account", () => {
      const id = pool.addAccount(createValidJwt({ accountId: "a1-existing", planType: "plus" }));

      pool.updateToken(id, createValidJwt({ accountId: "a1-existing", planType: "free" }));

      expect(pool.getEntry(id)?.planType).toBe("plus");
    });

    it("clears stale rate limit status when fresh quota allows requests", () => {
      const id = pool.addAccount(createValidJwt({ accountId: "a1-clear", planType: "plus" }));
      pool.markRateLimited(id, { retryAfterSec: 7200 });

      pool.updateCachedQuota(id, makeQuota({ rate_limit: {
        allowed: true,
        limit_reached: false,
        used_percent: 12,
        reset_at: Math.floor(Date.now() / 1000) + 3600,
    ...
    it("keeps rate limit status when secondary quota is exhausted", () => {
      const id = pool.addAccount(createValidJwt({ accountId: "a1-secondary", planType: "plus" }));
      pool.markRateLimited(id, { retryAfterSec: 7200 });
