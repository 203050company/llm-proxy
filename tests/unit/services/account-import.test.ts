...
describe("AccountImportService", () => {
  describe("importMany", () => {
    ...
    it("preserves id token returned from refresh-token exchange", async () => {
      const pool = makePool();
      const refreshedTokens = {
        access_token: createValidJwt({ accountId: "rt-id-token" }),
        refresh_token: "new_rt",
      ...
      const svc = new AccountImportService(
        ...
        makeScheduler(),
        makeDeps({
          refreshToken: async () => refreshedTokens,
        }),
      );

      await svc.importMany([{ refreshToken: "old_rt" }]);

      const entries = pool.getAllEntries();
      expect(entries[0].idToken).toBe("real-id-token");
    });
