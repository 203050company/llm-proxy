...
describe("self-update", () => {
  ...
  describe("checkProxySelfUpdate (docker mode)", () => {
    beforeEach(() => {
      // No .git → docker mode
      _existsSync.mockImplementation((path?: string) => !String(path ?? "").endsWith("/.git"));
    });
    ...
    it("skips registry checks for local docker builds", async () => {
      _readFileSync.mockImplementation((path: string) => {
        if (path === "/mock/.docker-build-source") return "local\n";
    ...
    it("returns release when update available in registry", async () => {
      _readFileSync.mockImplementation((path: string) => {
        if (path === "/mock/.docker-build-source") return "ghcr\n";
        return JSON.stringify({ version: "1.0.0" });
      });
    ...
    it("no false positive: registry has same version even if GitHub Release is newer", async () => {
      ...
      _readFileSync.mockImplementation((path: string) => {
        if (path === "/mock/.docker-build-source") return "ghcr\n";
        return JSON.stringify({ version: "2.0.44" });
      });
