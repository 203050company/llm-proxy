...
describe("E2E: Gemini endpoints", () => {
  ...
  it("all rate_limited accounts: returns 503 UNAVAILABLE", async () => {
    const allLimited = buildApp();
    try {
      const entryId = allLimited.accountPool.getAccounts()[0]?.id;
      expect(entryId).toBeDefined();
      allLimited.accountPool.markRateLimited(entryId!, { retryAfterSec: 3600 });

      const res = await allLimited.app.request("/v1beta/models/codex:generateContent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(defaultBody()),
      });
      ...
      allLimited.cookieJar.destroy();
      allLimited.proxyPool.destroy();
      allLimited.accountPool.destroy();
    }
  });
});
