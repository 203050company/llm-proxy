...
# 6) Stamp build metadata for update detection (COPY . invalidates cache, so this is always fresh)
RUN printf '%s\n' "$DOCKER_SOURCE" > /app/.docker-build-source && \
    date -u +%Y-%m-%dT%H:%M:%SZ > /app/.docker-build-time
