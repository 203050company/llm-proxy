...
export class AccountImportService {
  ...
  async importMany(entries: ImportEntry[]): Promise<ImportResult> {
    ...
    for (const entry of entries) {
      ...
      const entryId = this.pool.addAccount(resolved.token, resolved.rt, resolved.idToken);
  ...
  private async resolveToken(
    ...
    | { ok: true; token: string; rt: string | null; idToken: string | null }
    | { ok: false; error: string; kind: "validation" | "refresh_failed" }
  > {
    if (token) {
      ...
      return { ok: true, token, rt, idToken: null };
    }
    ...
    if (existing) {
      return { ok: true, token: existing.token, rt: existing.refreshToken, idToken: existing.idToken };
    }
