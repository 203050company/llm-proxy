...
export interface ProxySelection {
  mode: SpecialAssignment | "specific";
...
export class ProxyPool {
  ...
  /**
   * Resolve the proxy URL and safe display metadata for an account.
   * Returns:
   *   undefined — use global proxy (default behavior)
   *   null     — direct connection (no proxy)
   *   string   — specific proxy URL
   */
  resolveProxySelection(accountId: string, skipUnhealthy = false): ProxySelection {
    const assignment = this.getAssignment(accountId);

    if (assignment === "global") {
      return { mode: "global", proxyUrl: undefined, proxyId: null, proxyName: "Global Default" };
    }
    if (assignment === "direct") {
      return { mode: "direct", proxyUrl: null, proxyId: null, proxyName: "Direct (No Proxy)" };
    }

    if (assignment === "auto") {
      const picked = this.pickRoundRobin();
      return picked ?? {
        mode: "auto",
        proxyUrl: undefined,
        proxyId: null,
        proxyName: null,
        fallbackReason: "no_active_proxies",
      };
    }
    ...
    if (!proxy) {
      return {
        ...
        proxyId: assignment,
        proxyName: null,
        fallbackReason: "proxy_not_found",
      };
    }
    ...
    if (proxy.status === "unreachable" && skipUnhealthy) {
      return {
        ...
        proxyId: proxy.id,
        proxyName: proxy.name,
        fallbackReason: "proxy_unreachable",
      };
    }
    return {
      mode: "specific",
  ...
  resolveProxyUrl(accountId: string, skipUnhealthy = false): string | null | undefined {
    return this.resolveProxySelection(accountId, skipUnhealthy).proxyUrl;
  }
   ...
   */
  private pickRoundRobin(): ProxySelection | undefined {
