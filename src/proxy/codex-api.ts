...
import {
  parseRateLimitsEvent as parseRateLimitEvent,
...
import { normalizeRequestInputImages } from "./input-image-normalizer.js";
...
export class CodexApi {
  ...
  async warmupSession(model = "gpt-5.4-mini"): Promise<ParsedRateLimit | null> {
    ...
    try {
      ...
      let buffer = "";
      let lastRateLimit: ParsedRateLimit | null = null;
      ...
      const processBlock = (block: string): boolean => {
        ...
        if (event.event === "codex.rate_limits") {
          ...
          if (rateLimit) {
            lastRateLimit = rateLimit.code_review == null
              ? { primary: rateLimit.primary, secondary: rateLimit.secondary }
              : rateLimit;
          }
        }

        return ["response.completed", "response.failed", "response.cancelled", "error"].includes(event.event);
      };
  ...
  async createResponse(
    ...
    const normalizedRequest = await normalizeRequestInputImages(request);

    if (normalizedRequest.useWebSocket) {
      try {
        return await this.createResponseViaWebSocket(normalizedRequest, signal, onRateLimits, poolCtx);
      } catch (err) {
        ...
        const { previous_response_id: _, useWebSocket: _ws, ...httpRequest } = normalizedRequest;
        return this.createResponseViaHttp(httpRequest as CodexResponsesRequest, signal);
      }
    }
    return this.createResponseViaHttp(normalizedRequest, signal);
  }
