...
import { createEmptyRefreshSummary } from "./account-persistence.js";
...
export class AccountRegistry {
  ...
  addAccount(token: string, refreshToken?: string | null, idToken?: string | null): string {
    ...
    for (const existing of this.accounts.values()) {
      if (accountId) {
        if (existing.accountId === accountId && existing.userId === userId) {
          ...
          existing.planType = this.resolvePlanType(
            existing.planType,
            profile?.chatgpt_plan_type ?? null,
            existing.cachedQuota?.plan_type ?? null,
          );
          existing.status = isTokenExpired(token) ? "expired" : "active";
          existing.refreshSummary = createEmptyRefreshSummary();
          this.persistNow();
          return existing.id;
        }
    ...
    const entry: AccountEntry = {
      ...
      cachedQuota: null,
      quotaFetchedAt: null,
      refreshSummary: createEmptyRefreshSummary(),
    };
  ...
  updateToken(entryId: string, newToken: string, refreshToken?: string, idToken?: string | null): void {
    ...
    entry.planType = this.resolvePlanType(
      entry.planType,
      profile?.chatgpt_plan_type ?? null,
      entry.cachedQuota?.plan_type ?? null,
    );
  ...
  updateRefreshSummary(entryId: string, patch: Partial<RefreshSummary>): void {
    const entry = this.accounts.get(entryId);
    if (!entry) return;
    entry.refreshSummary = {
  ...
  recordRefreshAttempt(entryId: string, trigger: RefreshTrigger): void {
    this.updateRefreshSummary(entryId, {
      lastRefreshAttemptAt: new Date().toISOString(),
      lastRefreshTrigger: trigger,
    });
  }

  recordRefreshSuccess(entryId: string, trigger: RefreshTrigger): void {
  ...
  recordRefreshFailure(entryId: string, trigger: RefreshTrigger, failureCode: string | null, failureMessage: string | null): void {
    const now = new Date().toISOString();
    this.updateRefreshSummary(entryId, {
      lastRefreshAttemptAt: now,
      lastRefreshFailureAt: now,
      lastRefreshFailureCode: failureCode,
      lastRefreshFailureMessage: failureMessage,
      lastRefreshTrigger: trigger,
    });
  }
  ...
  isAuthenticated(): boolean {
    ...
    for (const entry of this.accounts.values()) {
      ...
      if (entry.status === "active" || entry.status === "rate_limited" || entry.status === "quota_exhausted") return true;
    }
    return false;
  }
  ...
  updateCachedQuota(entryId: string, quota: CodexQuota): void {
    ...
    entry.cachedQuota = quota;
    entry.quotaFetchedAt = new Date().toISOString();
    const quotaPlan = this.normalizePlanType(quota.plan_type);
    if (quotaPlan) {
      entry.planType = quotaPlan;
    }
    if (
      (entry.status === "quota_exhausted" || entry.status === "rate_limited") &&
  ...
  updatePlanType(entryId: string, planType: string | null): void {
    const entry = this.accounts.get(entryId);
    if (!entry) return;
    if (entry.planType === planType) return;
  ...
  toInfo(entry: AccountEntry): AccountInfo {
    ...
    const info: AccountInfo = {
      ...
      lastRefreshAttemptAt: refreshSummary.lastRefreshAttemptAt,
      lastRefreshSuccessAt: refreshSummary.lastRefreshSuccessAt,
      lastRefreshFailureAt: refreshSummary.lastRefreshFailureAt,
      lastRefreshFailureCode: refreshSummary.lastRefreshFailureCode,
      lastRefreshTrigger: refreshSummary.lastRefreshTrigger,
    };
  ...
  private normalizePlanType(planType: string | null | undefined): string | null {
  ...
  private resolvePlanType(
    ...
    const normalizedQuotaPlan = this.normalizePlanType(quotaPlan);
    if (normalizedQuotaPlan) return normalizedQuotaPlan;

    const normalizedJwtPlan = this.normalizePlanType(jwtPlan);
    if (!normalizedJwtPlan) return currentPlan;
    if (!currentPlan) return normalizedJwtPlan;
    if (currentPlan === normalizedJwtPlan) return currentPlan;

    // Access-token claims can report paid accounts as "free"; official quota
    // fetches are authoritative for downgrades.
    if (currentPlan !== "free" && normalizedJwtPlan === "free") {
      return currentPlan;
    }

    return normalizedJwtPlan;
  }

  private quotaAllowsRequests(quota: CodexQuota): boolean {
    return (
      quota.rate_limit.allowed &&
