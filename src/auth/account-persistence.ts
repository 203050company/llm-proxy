...
import type { AccountEntry, AccountsFile, CodexQuota, RefreshSummary } from "./types.js";
...
function loadPersisted(): {
  ...
  try {
    ...
    for (const entry of data.accounts) {
      ...
      // Backfill idToken for entries saved before OIDC ID token support.
      if ((entry as Partial<AccountEntry>).idToken === undefined) {
        entry.idToken = null;
        needsPersist = true;
      }
      ...
      if ((entry as Partial<AccountEntry>).refreshSummary === undefined) {
        entry.refreshSummary = createEmptyRefreshSummary();
        needsPersist = true;
      } else {
