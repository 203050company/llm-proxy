...
export class AccountPool {
  ...
  addAccount(token: string, refreshToken?: string | null, idToken?: string | null): string {
    return this.registry.addAccount(token, refreshToken, idToken);
  }
  ...
  updateToken(entryId: string, newToken: string, refreshToken?: string, idToken?: string | null): void {
    this.registry.updateToken(entryId, newToken, refreshToken, idToken);
  ...
  updateRefreshSummary(entryId: string, patch: Partial<RefreshSummary>): void {
    this.registry.updateRefreshSummary(entryId, patch);
  }

  recordRefreshAttempt(entryId: string, trigger: RefreshTrigger): void {
    this.registry.recordRefreshAttempt(entryId, trigger);
  }

  recordRefreshSuccess(entryId: string, trigger: RefreshTrigger): void {
    this.registry.recordRefreshSuccess(entryId, trigger);
  }
  ...
  recordRefreshFailure(
    ...
    this.registry.recordRefreshFailure(entryId, trigger, failureCode, failureMessage);
  }
  ...
  /** @deprecated Use applyRateLimit429() for new routing code. */
  markRateLimited(
    entryId: string,
    options?: { retryAfterSec?: number; resetsAtSec?: number; countRequest?: boolean },
  ): void {
    this.applyRateLimit429(entryId, options);
    const entry = this.registry.getEntry(entryId);
    if (!entry) return;
    const resetAt = entry.cachedQuota?.rate_limit.reset_at;
    entry.usage.rate_limit_until = resetAt != null
      ? new Date(resetAt * 1000).toISOString()
      : null;
    this.markStatus(entryId, "rate_limited");
  }
  ...
  updatePlanType(entryId: string, planType: string | null): void {
    this.registry.updatePlanType(entryId, planType);
  }
