...
import { CliAuthError, CodexCliAuthService } from "../../services/codex-cli-auth.js";
...
export function createCodexCliAuthRoutes(accountPool: AccountPool): Hono {
  const app = new Hono();
  const service = new CodexCliAuthService(accountPool);

  app.get("/admin/codex-cli-auth", (c) => c.json(service.getStatus()));

  app.post("/admin/codex-cli-auth/apply", async (c) => {
