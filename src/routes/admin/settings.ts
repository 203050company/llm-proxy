...
export function createSettingsRoutes(accountPool?: AccountPool, proxyPool?: ProxyPool): Hono {
  ...
  app.post("/admin/gemini-settings", async (c) => {
    if (!hasAdminWriteAuth(c)) {
      c.status(401);
      return c.json({ error: "Invalid current API key" });
    }
    ...
    if (body.api_key_priority !== undefined && body.api_key_priority !== "api_key" && body.api_key_priority !== "oauth") {
      c.status(400);
      return c.json({ error: "api_key_priority must be api_key or oauth" });
    }
    if (body.refresh_margin_seconds !== undefined && (!Number.isInteger(body.refresh_margin_seconds) || body.refresh_margin_seconds < 0)) {
      c.status(400);
      return c.json({ error: "refresh_margin_seconds must be an integer >= 0" });
    }
    if (body.refresh_concurrency !== undefined && (!Number.isInteger(body.refresh_concurrency) || body.refresh_concurrency < 1)) {
      c.status(400);
    ...
    mutateYaml(getLocalConfigPath(), (data) => {
      ...
      const gemini = data.gemini as Record<string, unknown>;
      for (const key of [
        "oauth_enabled",
        "credentials_path",
        ...
        if (body[key] !== undefined) gemini[key] = body[key];
      }
    });
    reloadAllConfigs();

    const updated = getConfig();
    return c.json({ success: true, ...updated.gemini });
  });
  ...
  app.post("/admin/general-settings", async (c) => {
    ...
    const oldPort = currentConfig.server.port;
    const oldDefaultModel = currentConfig.model.default;
