...
async function handleCompact(
  ...
  if (compactRouteMatch?.kind === "api-key" || compactRouteMatch?.kind === "adapter" || compactRouteMatch?.kind === "gemini-oauth") {
    ...
    const apiKeyEntryId = compactRouteMatch.kind === "api-key" ? compactRouteMatch.entry.id : undefined;
    return handleDirectRequest({ c, upstream: compactRouteMatch.adapter, req: directReq, fmt: PASSTHROUGH_FORMAT, apiKeyPool, apiKeyEntryId });
  }
...
export function createResponsesRoutes(
  ...
  const responsesHandler = async (c: Context) => {
    ...
    if (routeMatch?.kind === "api-key" || routeMatch?.kind === "adapter" || routeMatch?.kind === "gemini-oauth") {
      ...
      return handleDirectRequest({ c, upstream: routeMatch.adapter, req: directReq, fmt: PASSTHROUGH_FORMAT, apiKeyPool, apiKeyEntryId });
    }
  ...
  const compactHandler = async (c: Context) => {
    ...
    return handleCompact(c, accountPool, cookieJar, proxyPool, body, upstreamRouter, apiKeyPool);
  };
