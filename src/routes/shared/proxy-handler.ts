...
function parseRetryAfterMs(err: unknown): number | null {
  if (!(err instanceof CodexApiError) || err.status !== 429) return null;
  const match = err.message.match(/reset after\s+(\d+)s/i) ??
    err.body.match(/reset after\s+(\d+)s/i);
...
function recordUpstreamEmptyResponse(upstream: UpstreamAdapter): void {
...
export async function handleDirectRequest(options: HandleDirectRequestOptions): Promise<Response> {
  const { c, upstream, req, fmt, apiKeyPool, apiKeyEntryId } = options;
  const abortController = new AbortController();
  c.req.raw.signal.addEventListener("abort", () => abortController.abort(), { once: true });
    ...
    for (;;) {
      ...
      try {
        ...
        if (retryAfterMs != null && rateLimitRetries < MAX_EMPTY_RETRIES) {
          rateLimitRetries++;
          console.warn(`[${fmt.tag}] Direct request hit 429, waiting ${retryAfterMs}ms before retrying...`);
        ...
        if (err instanceof CodexApiError) {
          ...
          if (canReturnStreamError(req, fmt)) {
            return { ok: false, response: streamErrorResponse(c, fmt, code, err.message) };
          }
          c.status(code);
          try {
            const parsed: unknown = JSON.parse(err.body);
            if (parsed && typeof parsed === "object") {
              return { ok: false, response: c.json(parsed) };
            }
          } catch { /* non-JSON body — fall through */ }
          if (code === 429) {
            return { ok: false, response: c.json(fmt.format429(err.message)) };
          }
          return { ok: false, response: c.json(fmt.formatError(code, err.message)) };
        }

        if (canReturnStreamError(req, fmt)) {
          return { ok: false, response: streamErrorResponse(c, fmt, 502, msg) };
        }
        c.status(502);
        return { ok: false, response: c.json(fmt.formatError(502, msg)) };
      }
    }
  };

  const initial = await createRawResponse();
  if (!initial.ok) return initial.response;
  let rawResponse = initial.response;
  ...
  if (req.isStreaming) {
    ...
    return stream(c, async (s) => {
      ...
      if (apiKeyPool && apiKeyEntryId && usageInfo) {
        apiKeyPool.recordUsage(apiKeyEntryId, usageInfo);
      }
      recordUpstreamSuccessfulResponse(upstream);
    });
  }

  // Non-streaming
  let emptyRetriesForModel = 0;
  try {
    for (;;) {
      try {
        const result = await fmt.collectTranslator(upstream, rawResponse, req.model, req.tupleSchema);
        if (apiKeyPool && apiKeyEntryId && result.usage) {
          apiKeyPool.recordUsage(apiKeyEntryId, result.usage);
        }
        recordUpstreamSuccessfulResponse(upstream);
        return c.json(result.response);
      } catch (err) {
        if (err instanceof EmptyResponseError && emptyRetriesForModel < MAX_EMPTY_RETRIES) {
          emptyRetriesForModel++;
          recordUpstreamEmptyResponse(upstream);
          if (apiKeyPool && apiKeyEntryId && err.usage) {
            apiKeyPool.recordUsage(apiKeyEntryId, err.usage);
          }
          const retry = await createRawResponse();
          if (!retry.ok) return retry.response;
          rawResponse = retry.response;
          continue;
        }
        throw err;
      }
