...
export function createProxyRoutes(
  ...
  app.post("/api/proxies/assign", async (c) => {
    ...
    const normalizedAccountId = normalizeAssignmentAccountId(accountId);
    proxyPool.assign(normalizedAccountId, proxyId);
    return c.json({
      success: true,
      assignment: { accountId, proxyId, normalizedAccountId },
      displayName: proxyPool.getAssignmentDisplayName(normalizedAccountId),
    });
  });

  // Unassign proxy from account
  app.delete("/api/proxies/assign/:accountId", (c) => {
    const accountId = normalizeAssignmentAccountId(c.req.param("accountId"));
  ...
  app.get("/api/proxies/assignments", (c) => {
    const codexAccounts = accountPool.getAccounts().map((a) => {
      const id = `codex:${a.id}`;
      return {
        id,
        provider: "codex",
        email: a.email,
        status: a.status,
        proxyId: getAssignmentWithLegacyFallback(proxyPool, id, a.id),
  ...
  app.post("/api/proxies/assign-bulk", async (c) => {
    ...
    proxyPool.bulkAssign(body.assignments.map((assignment) => ({
      ...assignment,
      accountId: normalizeAssignmentAccountId(assignment.accountId),
    })));
  ...
  app.post("/api/proxies/assign-rule", async (c) => {
    ...
    for (let i = 0; i < body.accountIds.length; i++) {
      assignments.push({
        accountId: normalizeAssignmentAccountId(body.accountIds[i]),
  ...
  app.post("/api/proxies/assignments/import", async (c) => {
    ...
    const emailToAccount = new Map<string, { namespacedId: string }>([
    ...
    for (const { email, proxyId } of body.assignments) {
      ...
      const currentProxyId = proxyPool.getAssignment(account.namespacedId);
      if (currentProxyId === proxyId) {
        unchanged++;
      } else {
        changes.push({
          email,
          accountId: account.namespacedId,
          from: currentProxyId,
          to: proxyId,
        });
      }
...
function getAssignmentWithLegacyFallback(
  proxyPool: ProxyPool,
  namespacedId: string,
  legacyId: string,
): string {
  const current = proxyPool.getAssignment(namespacedId);
  if (current !== "global") return current;
  return proxyPool.getAssignment(legacyId);
}

function getAssignmentDisplayNameWithLegacyFallback(
  proxyPool: ProxyPool,
  namespacedId: string,
  legacyId: string,
): string {
  const current = proxyPool.getAssignment(namespacedId);
  if (current !== "global") return proxyPool.getAssignmentDisplayName(namespacedId);
  return proxyPool.getAssignmentDisplayName(legacyId);
}
