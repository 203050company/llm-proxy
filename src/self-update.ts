...
export type DockerBuildSource = "local" | "ghcr" | "unknown";
...
function getDockerBuildSource(): DockerBuildSource {
  try {
    const source = readFileSync(resolve(getRootDir(), ".docker-build-source"), "utf-8").trim();
    return source === "local" || source === "ghcr" ? source : "unknown";
  } catch {
    return "unknown";
  }
}
...
export async function checkProxySelfUpdate(): Promise<ProxySelfUpdateResult> {
  ...
  if (mode === "docker") {
    const dockerSource = getDockerBuildSource();
    if (dockerSource === "local") {
      const result: ProxySelfUpdateResult = {
        commitsBehind: 0, currentCommit: null, latestCommit: null,
        commits: [], changelog: null, release: null,
    ...
    const updateAvailable = registryVersion !== null
