...
export interface CodexModelInfo {
  ...
  /** Raw input context window reported by the Codex backend, when known. */
...
export class ModelStore {
        ...
        if (cachedModels.length > 0) {
          ...
          for (const m of cachedModels) {
            if (!staticIds.has(m.id)) {
              this.catalog.push(normalizeCachedModel(m));
              added++;
            }
          }
          if (added > 0) {
  ...
  resolveModelId(input: string): string {
    const trimmed = stripClaudeCodeContextSuffix(input.trim());
...
function normalizeCachedModel(model: CodexModelInfo): CodexModelInfo {
  if (
    model.source !== "static" &&
    model.contextWindow !== undefined &&
    model.inputContextWindow === undefined &&
    model.maxOutputTokens !== undefined &&
    (model.maxContextWindow === undefined || model.maxContextWindow === model.contextWindow)
  ) {
    return {
      ...model,
      contextWindow: model.contextWindow + model.maxOutputTokens,
      inputContextWindow: model.contextWindow,
      source: "backend",
    };
  }

  return { ...model, source: "backend" as const };
}

function inferMaxOutputTokens(id: string, contextWindow?: number): number | undefined {
  return /^gpt-5(?:\.|-|$)/.test(id) && contextWindow === 272000 ? 128000 : undefined;
}

function normalizeBackendModel(raw: BackendModelEntry): NormalizedModelWithMeta {
  const id = raw.slug ?? raw.id ?? raw.name ?? "unknown";
  const rawContextWindow = typeof raw.context_window === "number"
    ? raw.context_window
  ...
  const maxOutputTokens = typeof raw.max_output_tokens === "number"
    ...
    : typeof raw.maxOutputTokens === "number"
      ? raw.maxOutputTokens
      : inferMaxOutputTokens(id, rawContextWindow);
  ...
  if (rawContextWindow !== undefined) {
    const contextLooksLikeInputWindow =
      maxOutputTokens !== undefined &&
      (out.maxContextWindow === undefined || out.maxContextWindow === rawContextWindow);
    out.contextWindow = contextLooksLikeInputWindow
      ? rawContextWindow + maxOutputTokens
      : rawContextWindow;
    if (contextLooksLikeInputWindow) out.inputContextWindow = rawContextWindow;
  }
