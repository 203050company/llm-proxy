...
type AddProvider = "codex" | null;

interface AddAccountProps {
  visible: boolean;
  provider: AddProvider;
  onChooseProvider: (provider: "codex") => void;
  onStartCodex: () => Promise<void>;
  onCancel: () => void;
  onSubmitRelay: (provider: "codex", callbackUrl: string) => Promise<void>;
        ...
        <section class="bg-white dark:bg-card-dark border border-gray-200 dark:border-border-dark rounded-xl p-5 shadow-sm transition-colors space-y-4">
          ...
          {!provider && (
            <button
              ...
              class="w-full px-4 py-4 text-left rounded-lg border border-gray-200 dark:border-border-dark hover:border-primary/50 hover:bg-slate-50 dark:hover:bg-border-dark transition-colors"
            >
