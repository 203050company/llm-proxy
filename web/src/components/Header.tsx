...
function StableText({ tKey, children, class: cls }: { tKey: TranslationKey; children: string; class?: string }) {
  return (
    <span class={`inline-grid ${cls ?? ""}`}>
      ...
      <span class="invisible col-start-1 row-start-1 whitespace-nowrap">{translations.ko[tKey]}</span>
        ...
        <div class="flex w-full max-w-[960px] items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              ...
              title={"EN/\u4e2d/KO"}
            >
              <span class="text-xs font-bold inline-flex items-center justify-center w-5">
                {lang === "en" ? "EN" : lang === "zh" ? "\u4e2d" : "KO"}
              </span>
            </button>
            {/* Theme Toggle */}
            <button
