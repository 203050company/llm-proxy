...
export function AccountTable({
  ...
  const [logAccount, setLogAccount] = useState<AssignmentAccount | null>(null);
    ...
    <div class="flex flex-col gap-3">
      ...
      <div class="bg-white dark:bg-card-dark border border-gray-200 dark:border-border-dark rounded-xl overflow-hidden">
        ...
        <div class="flex items-center gap-3 px-4 py-2.5 border-b border-gray-100 dark:border-border-dark bg-slate-50 dark:bg-bg-dark text-xs text-slate-500 dark:text-text-dim font-medium">
          ...
          <span class="w-12 text-center">{t("accountLogs")}</span>
            ...
            return (
              ...
              >
                ...
                <span class="flex-1 min-w-0">
                  <span class="block text-sm font-medium truncate text-slate-700 dark:text-text-main">
                    {acct.label ? `${acct.label} (${acct.email})` : acct.email}
                  </span>
                  <AccountRefreshStatus account={acct} compact />
                </span>
                ...
                <span class="w-12 flex justify-center" onClick={(e) => e.stopPropagation()}>
                  <button
                    onClick={() => setLogAccount(acct)}
                    class="p-1.5 text-slate-400 dark:text-text-dim hover:text-primary rounded-md hover:bg-slate-100 dark:hover:bg-border-dark transition-colors"
                    title={t("viewLogs")}
                  >
      ...
      {logAccount && (
        <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 px-4" onClick={() => setLogAccount(null)}>
          <div
            class="w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-xl bg-white dark:bg-card-dark border border-gray-200 dark:border-border-dark shadow-xl p-4"
            onClick={(e) => e.stopPropagation()}
          >
            <AccountRefreshStatus account={logAccount} />
            <AccountLogPanel account={logAccount} onClose={() => setLogAccount(null)} />
          </div>
        </div>
      )}
    </div>
