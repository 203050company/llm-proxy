...
const VIEW_MODE_STORAGE_KEY = "codex-proxy-account-list-view-mode";
...
export function AccountList({ accounts, loading, onDelete, onRefresh, refreshing, lastUpdated, proxies, onProxyChange, onExport, onImport, onToggleStatus, onUpdateLabel, apiKey }: AccountListProps) {
  const t = useT();
  const { lang } = useI18n();
  ...
  const handleApplyToCli = useCallback(async (id: string) => {
    const path = cliAuth.status?.path ?? "~/.codex/auth.json";
    if (!confirm(t("applyToCodexCliConfirm").replace("{path}", path))) return null;
    const err = await cliAuth.apply(id);
    if (err) {
      setCliMessage({ text: t("applyToCodexCliFailed").replace("{error}", err), error: true });
      setTimeout(() => setCliMessage(null), 6000);
      return err;
    }
    setCliMessage({ text: t("applyToCodexCliSuccess") });
  ...
  return (
    <section class="flex flex-col gap-4">
      ...
      <div class={viewMode === "list" ? "overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-border-dark dark:bg-card-dark" : "grid grid-cols-1 md:grid-cols-2 gap-4"}>
        {loading ? (
          ...
          viewMode === "list" ? (
            <>
              <div class="hidden md:grid grid-cols-[minmax(0,1.7fr)_9rem_8rem_8rem_minmax(12rem,1fr)] gap-3 px-4 py-2 text-[0.65rem] font-semibold uppercase tracking-wide text-slate-400 dark:text-text-dim border-b border-gray-100 dark:border-border-dark/60">
                <span>{t("accountIdentifier")}</span>
                <span>{t("authStatus")}</span>
                <span>{t("current5hUsage")}</span>
                <span>{t("weeklyUsage")}</span>
                <span class="text-right">{t("actions")}</span>
              </div>
              {displayAccounts.slice(0, visibleCount).map((acct) => (
                <AccountOverviewListItem
                  ...
                  selected={selectedIds.has(acct.id)}
                  onToggleSelect={toggleSelect}
                  onRefreshQuota={async (id) => { await fetch(`/auth/accounts/${encodeURIComponent(id)}/refresh`, { method: "POST" }); onRefresh(); }}
